...
const auth = new GoogleAuth({
  credentials: {
    client_email: process.env.PM_STATION_FIREBASE_CLIENT_EMAIL,
    private_key: (
...
const drive = async () => {
  ...
  if (!targetClient) {
    console.log(process.env.PM_STATION_KIOSK_SERVICE_ACCOUNT);
    targetClient = new Impersonated({
      sourceClient: client as JSONClient,
      targetPrincipal: process.env.PM_STATION_KIOSK_SERVICE_ACCOUNT,
      lifetime: 10 * 60,
      delegates: [],
      targetScopes: scopes,
    });
