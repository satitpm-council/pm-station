...
export const getTodayPlaylist = async (): Promise<ValidatedDocument<TypeOf<typeof PlaylistRecord>>> => {
  const date = dayjs()
    .tz("Asia/Bangkok")
    .date(6)
    .hour(7)
  ...
  try {
    return { ...PlaylistRecord.parse(docs[0].data()), id: docs[0].id, exists: true, validated: true};
  } catch (err) {
    throw new Error(
